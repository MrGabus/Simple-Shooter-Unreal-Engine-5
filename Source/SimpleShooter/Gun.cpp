...
{
	ParticleAndSoundEffectsSpawn();
	FHitResult Hit;
	FVector ShotDirection;
	bool bSucess = GunTrace(Hit, ShotDirection);
	if (bSucess && GetOwnerController())
	{				
		ParticleAndSoundEffectsHit(Hit, ShotDirection);
...
void AGun::ParticleAndSoundEffectsHit(FHitResult& Hit, FVector& ShotDirection)
{
	if (HitParticle)
	{
...
void AGun::ParticleAndSoundEffectsSpawn()
{
	if (MuzzleFlash)
	{
